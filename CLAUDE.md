...
ディレクトリではなく**ファイル分割**で表現します。`transport`（HTTPハンドラー）だけは、
...
└── transport/                 # package <name>http: HTTPハンドラー（Gin）
    └── handler.go
```
...
**注意**: Goの慣例に従い、**リポジトリインターフェースは利用者側のファイル**（`usecase.go` / `transport/handler.go`）で定義します。別途 domain/repository ディレクトリには配置しません。
...
2. **api 型境界**: フィーチャーコアは `internal/api` をインポート不可（`transport` 層のみ可）
...
5. **transport層を追加**:
   - `transport/handler.go` - HTTPハンドラー（`package <name>http`。必要に応じてusecaseインターフェースもここで定義可）
