...
> **注記 (2026-05-24)**: PostgreSQL の採用方針自体は維持されているが、当 ADR で
> 言及している ORM 選定（GORM）は [ADR-0006](0006-db操作をgormからsqlcとgooseへ移行.md)
> によって sqlc + goose に置き換えられた。本文中の GORM 関連の記述は歴史的記録として
> 残してあるが、現行構成ではない点に注意。
