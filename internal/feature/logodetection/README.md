...
graph TB
    ...
    subgraph "Usecase Layer"
        Usecase[logodetectionUsecase<br/>usecase.go]
...
#### ドメイン層
- **DetectedLogo**（[logo.go](logo.go)）: `Name`（検出された企業名）、`Confidence`（信頼度スコア 0.0〜1.0）
- **CompanyAnalysis**（[analysis.go](analysis.go)）: `CompanyName`（分析対象の企業名）、`Summary`（AI生成の分析サマリー）

#### アダプター層 - Vision（[vision/client.go](vision/client.go)）
...
#### アダプター層 - Gemini（[gemini/client.go](gemini/client.go)）
