...
AGENTS.md のアーキテクチャルールに基づき、以下を検査する：

- **依存関係ルール違反**: フィーチャーコアが他フィーチャーや `internal/api` をインポートしていないか（`go tool go-arch-lint check` で確認）
- **レイヤー責務の混在**: ビジネスロジックが handler に書かれていないか、HTTP固有の処理が usecase に入っていないか
- **リポジトリインターフェースの配置**: インターフェースが利用者側のファイル（usecase.go / <name>http/handler.go）で定義されているか
- **API型とエンティティの混同**: OpenAPI生成型（`internal/api`）を usecase やドメイン層で使っていないか（API型は <name>http 層のみで使用）
